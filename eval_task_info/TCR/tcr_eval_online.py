...
def default_output_path(tasks_path: Path, suffix: str) -> Path:
    if tasks_path.expanduser().is_dir():
        return Path(f"tcr_batch_results{suffix}")
    return Path(f"{tasks_path.stem}_tcr_results{suffix}")
...
def build_target_messages(task: Dict[str, Any], full_session_text: str, full_session_path: Path, task_file: Path) -> List[Dict[str, str]]:
    ...
    system = (
        "你是一个校园心理咨询案例时序—因果推理测试中的被测大模型。"
        "请只依据提供的完整咨询历史回答问题。"
        "需要按题目要求分析事件的时间顺序、相互影响和核心困扰的演化过程。"
        "不要编造未在材料中出现的事实、人物、诊断、数值或因果关系。"
        "若材料中没有足够依据，请明确说“不确定/材料中未明确提到”。"
    )
    user = f"""请根据以下咨询历史回答问题。
...
def run(config: RunnerConfig) -> None:
    ...
    for idx, item in enumerate(items, start=1):
        ...
        try:
            ...
            if config.dry_run:
                model_answer = "[DRY RUN] 这里会是被测模型根据完整咨询历史和 question 生成的时序—因果分析回答。"
            else:
...
def parse_args(argv: Optional[Sequence[str]] = None) -> RunnerConfig:
    p = argparse.ArgumentParser(description="Run batch TCR evaluation tasks through OpenRouter/OpenAI SDK.")
    p.add_argument(
        "--tasks",
        default="TCR",
        help="任务 JSON 文件或目录。例如 TCR；目录下每个 *.json 都会被读取。默认：TCR",
    )
    p.add_argument("--rubric", required=True, help="评分规则 Markdown 文件，例如 评测.md")
    p.add_argument(
        "--full-session-dir",
        default=None,
    ...
    p.add_argument("--max-target-tokens", type=int, default=3000)
    ...
    p.add_argument("--title", default=os.environ.get("OPENROUTER_APP_TITLE", "Counseling TCR Batch Eval"), help="OpenRouter 可选 X-OpenRouter-Title")
    ...
    p.add_argument(
        ...
        action="store_true",
        help="评测模型不接收完整历史，只接收 reference_answer/evaluation_focus/被测回答。默认会接收完整历史以判断幻觉。",
    )
    p.add_argument(
